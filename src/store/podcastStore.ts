...
interface PodcastState {
  ...
  fetchBestPodcasts: (options?: { genre_id?: number; page?: number; region?: string; language?: string; sort_by?: string; sort_order?: string; page_size?: number; }) => Promise<void>;
  generateSummary: (podcastId: string, options: SummaryOptions & { length?: string }) => Promise<Summary | null>;
...
const usePodcastStore = create<PodcastState>()((set, get) => ({
  ...
  deletePodcast: (id: string) => {
    set((state) => {
      ...
      const { [id]: _, ...remainingPodcasts } = state.podcasts;
  ...
  fetchBestPodcasts: async (options: { genre_id?: number; page?: number; region?: string; language?: string; sort_by?: string; sort_order?: string; page_size?: number; } = {}) => {
    const { setError, setPodcasts } = get();
      ...
      const result = await podcastAPI.getBestPodcasts(options);
        ...
        const podcastsArr = Array.isArray(result.data) ? result.data : [];
        setPodcasts(podcastsArr);
              // Update categories from results
              const uniqueCategories = Array.from(
          new Set(podcastsArr.map((podcast: Podcast) => podcast.category))
        ).sort() as string[];
              set({ categories: uniqueCategories });
            }
          } catch (error) {
  ...
  generateSummary: async (podcastId, options: SummaryOptions & { length?: string }) => {
          const { 
            setLoadingPodcastId, 
            addSummary, 
            podcasts, 
